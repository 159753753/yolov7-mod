    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, EIoU=True, Focal=True) # iou(prediction, target)
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, WIoU=True, wiou_scale=True)
                ...
                # ComputeLossOTA类的call函数在train.py看hyp中用的是哪个yaml文件，在使用的yaml文件中在里面的loss_ota，如果为0则使用class ComputeLoss如果为1则使用class ComputeLossOTA默认为1
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, EIoU=True, Focal=True)  # iou(prediction, target)
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, WIoU=True, wiou_scale=True)
                # lbox += (1.0 - iou).mean()  # iou loss
                if type(iou) is tuple:
                    if len(iou) == 2:
                        lbox += (iou[1].detach().squeeze() * (1 - iou[0].squeeze())).mean()
