...
class WIoU_Scale:
    ...
    @classmethod  # 先调用@classmethod里，然后才WIoU_Scale构造函数初始化
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False,
             WIoU=False, wiou_scale=False, IoU=False, Focal=False, alpha=1, gamma=0.5, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                      # 不足：1.CIoU使用的是宽和高的相对比例，并不是宽和高的值。 2.w和h的梯度值具有相反的符号。其中一个值增大时，另外一个值必须减小，它俩不能保持同增同减。
                v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)  # atan(w/h)基本上在非饱和区，收敛快
                with torch.no_grad():
                    alpha_ciou = v / (v - iou + (1 + eps))  # 保持v不变，iou越大，alpha越大，即iou越好，长宽比权重越大
                if Focal:
                    return iou - (rho2 / c2 + torch.pow(v * alpha_ciou + eps, alpha)), torch.pow(inter / (union + eps),
                                                                                                 gamma)  # Focal_CIoU
                else:
                    return iou - (rho2 / c2 + torch.pow(v * alpha_ciou + eps, alpha))  # CIoU
            elif EIoU:  # 将w, h边长作为惩罚项
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=()):
    # prediction其shape是(2, 5000, 6)
    # 2表示的是图片的个数，5000表示是网络预测的候选框的个数，=[x y w h 含有目标的概率值 类别1概率 类别2概率 ...]
    ...
    nc = prediction.shape[2] - 5  # number of classes  减去不是类别概率的前5项
    xc = prediction[..., 4] > conf_thres  # candidates  [..., 4]=第5个数  True/False
    ...
    t = time.time()
    output = [torch.zeros((0, 6), device=prediction.device)] * prediction.shape[0]  # shape[0]图片个数
    for xi, x in enumerate(prediction):  # image index, image inference x = [5000,6]
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence  保留xc[xi]=True
        ...
        if nc == 1:
            ...
            # so there is no need to multiplicate.  冒号含左不含右 [4:5]=[4]
        else:
        ...
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T  # tensor.nonzero返回tensor矩阵中不为0元素的索引
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
            # x -> [5000,x1 y1 x2 y2 conf[不为0] class-id[]]
        else:  # best class only
            conf, j = x[:, 5:].max(1, keepdim=True)  # max的输入参数是1表示对每行求最大值 tensor.max -> val, index
            # conf.shape = torch.Size([5000, 1])
            x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]
            # x -> [5000,x1 y1 x2 y2 conf class-id]

        # Filter by class
        if classes is not None:
            x = x[(x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]
            # 筛选出指定的class
            # tensor.any 存在一个元素为True, 那么返回True; 只有所有元素都是False时才返回False, any(1) ->横  0->竖
        ...
        # 偏移量仅取决于类的ID（也就是x[:, 5:6]），并且 足 够 大 ，以便来自不同类的框不会重叠
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            ...
            weights = iou * scores[
                None]  # box weights [i,1]*[1,5000] # None is an alias for NP.newaxis. It creates an axis with length 1.
            x[i, :4] = torch.mm(weights, x[:, :4]).float() / weights.sum(1,
                                                                         keepdim=True)  # merged boxes [i,5000]*[5000,4] i输出box数 n检测出box数
            if redundant:
                i = i[iou.sum(1) > 1]  # require redundancy

        output[xi] = x[i]  # [:,x y x y 概率([]) 种类([])]
        if (time.time() - t) > time_limit:
            print(f'WARNING: NMS time limit {time_limit}s exceeded')
            break  # time limit exceeded

    return output  # [2,5000,x y x y 概率([]) 种类([])]


# soft_mns start
def box_iou_for_nms(box1, box2, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIou=False,
                    eps=1e-7):  # -setchipinychzz
    # Intersection over Union交并比=交集/并集
    ...
    inter = (b1_x2.minimum(b2_x2) - b1_x1.maximum(b2_x1)).clamp(0) * \
            (b1_y2.minimum(b2_y2) - b1_y1.maximum(b2_y1)).clamp(
                0)  # Clamps all elements in :attr:`input` into the range `[` :attr:`min`, :attr:`max` `]`.
