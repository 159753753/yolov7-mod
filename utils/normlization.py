...
if __name__ == '__main__':
    input = torch.randn(2, 8, 2, 2)  # batch_size*num_features*height*width
    print(input, '\n')
    bn = nn.BatchNorm2d(input.shape[1])
    output = bn(input)
    print(output, '\n', torch.var(output), '\n')

    cbn = CBatchNorm2d(input.shape[1])
    output = cbn(input, min(16 / 4, 8))  # cbn2d weight best min(8, 16 / bs per gpu)
    print(output, '\n', torch.var(output), '\n')
