...
def main(opts):
    ...
    if 'mp4' in opts.obj or 'MP4' in opts.obj:  # if it is a video
        ...
        get_next_frame = lambda _ : obj.read()  # lambda [arg1 [,arg2,.....argn]]:expression

        if os.path.isabs(obj_name):  # 检查指定的路径是否为绝对路径
            obj_name = obj_name.split('/')[-1][:-4]
        else: obj_name = obj_name[:-4]  # 去后缀
    ...
    while True:
        ...
        if frame_id == 0:
            print('out', len(out))  # 2 --v7x
            print('out[0]', out[0].shape)  # torch.Size([1, 15120, 85])
            print('out[1]', len(out[1]))  # 3
            print('out[1][0]', out[1][0].shape)  # torch.Size([1, 3, 48, 80, 85])
            print('out[1][1]', out[1][1].shape)  # torch.Size([1, 3, 24, 40, 85])
            print('out[1][2]', out[1][2].shape)  # torch.Size([1, 3, 12, 20, 85])
        ...
        if len(out.shape) == 3:  # case (bs, num_obj, ...)
            ...
            out = out.squeeze(0)  # 去除size为1的维度,squeeze(0)代表若第一维度值为1则去除第一维度，squeeze(1)代表若第二维度值为1则去除第二维度
...
class my_queue:
    ...
    """
    def __init__(self, arr: list) -> None:  # 类型建议符
        self.arr = arr 
        self.start_idx = 0
...
def post_process_v7(out, img_size, ori_img_size):
    ...
    out = non_max_suppression(out, conf_thres=0.01, )[0]  # NMS [:,:,x y x y 概率([]) 种类([])]
...
def preprocess_v7(ori_img, model_size, model_stride):
    ...
    img_resized = np.ascontiguousarray(img_resized)  # 内存连续
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='../weights/yolov7x.pt', help='model path')
